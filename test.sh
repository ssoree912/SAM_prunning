                                ...
                                for second_step_after_mask in 1 ; do #1이 원본 e 없는 상태로 mask 생성 
                                    for random_mask_change in 0 ; do
                                            for use_DLB in 1 ; do #에폭 절반  
                                                for DLB_loss in 1 ; do ## 겹치기는하는데 + sam 그래디언트 쓰기도하고 안쓰기도 하기
...
python -m torch.distributed.launch --nproc_per_node=1 --master_port=21888 --use_env cifar.py \
 ...
 > ./cifar_log/${ms}_${lr}_attn${attn_pr}_ffn${ffn_pr}_pf${prune_freq}_sam_rho${rho}_use_DLB${use_DLB}_DLB_loss_${DLB_loss}_overlap_${overlap}_mask_finding_cos${mask_finding_cos}.txt
                                 

checkpoint_path="cifar100_epoch2_${lr}_small_attn0.5_ffn0.5_8_sam_rho${rho}_use_DLB${use_DLB}_DLB_loss_${DLB_loss}_overlap_${overlap}_mask_finding_cos_${mask_finding_cos}.pth"
...
log_filename="./fine_log/${ms}_${checkpoint_basename}.txt"
                                    ...
                                    python -m torch.distributed.launch --nproc_per_node=1 --master_port=25555 --use_env cifar_fine.py \
                                        --data-set IMNET \
                                        --epochs 2 \
                                        --data-path /home/20223204/data/CIFAR100 \
