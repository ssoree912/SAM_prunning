...
python -m torch.distributed.launch --nproc_per_node=1 --master_port=21888 --use_env cifar.py \
 ...
 --DLB_loss "${DLB_loss}" \
 --overlap "${overlap}" \
 > ./cifar_log/${ms}_${lr}_attn${attn_pr}_ffn${ffn_pr}_pf${prune_freq}_sam_rho${rho}_use_DLB${use_DLB}_DLB_loss_${DLB_loss}_overlap_${overlap}.txt
                                 
        
            
checkpoint_path="cifar100_epoch25_5e-05_small_attn0.5_ffn0.5_8_sam_rho${rho}_use_DLB${use_DLB}_DLB_loss_${DLB_loss}_overlap_${overlap}.pth"
